...
class GameViewController: UIViewController {
    ...
    func startGame() {
        questionLabel.text = question.first?.question
        option1Button.setTitle(question.first?.option1 ?? "", for: UIControl.State())
        option2Button.setTitle(question.first?.option2 ?? "", for: UIControl.State())
        option3Button.setTitle(question.first?.option3 ?? "", for: UIControl.State())
        option4Button.setTitle(question.first?.option4 ?? "", for: UIControl.State())
    ...
    func nextQuestion() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
            if self.question.count > self.indexQuestion+1 {
